...
const localDataApiProxyTarget = process.env.LOCAL_VERCEL_API_ORIGIN?.trim() || "https://buy.pintecl.com";
const shouldProxyDataApi = process.env.NODE_ENV !== "production" && !process.env.MONGODB_URI;
...
async function startServer() {
  ...
  if (process.env.NODE_ENV !== "production") {
    const vite = await createViteServer({
      server: {
        middlewareMode: true,
        proxy: shouldProxyDataApi
          ? {
              "/api/data": { target: localDataApiProxyTarget, changeOrigin: true },
              "/api/quotation": { target: localDataApiProxyTarget, changeOrigin: true },
            }
          : undefined,
      },
      appType: "spa",
    ...
    if (shouldProxyDataApi) {
      console.log(`Local data and quotation APIs proxy to ${localDataApiProxyTarget}.`);
    }
