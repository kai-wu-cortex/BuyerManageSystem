...
import { createSessionCookie, createSessionToken } from './sessionAuth.ts';
...
export async function handleLoginRequest(req: ApiRequest, res: ApiResponse): Promise<unknown> {
  ...
  if (!sessionSecret) {
    return res.status(503).json({ success: false, code: 'SESSION_NOT_CONFIGURED', message: '登录会话尚未配置。' });
  }
