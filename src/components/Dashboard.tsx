    ...
    'kpis': {
      ...
      content: (
        <div className="space-y-3 pointer-events-auto">
          {/* 数据过滤工具栏 */}
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl bg-slate-50/80 px-3 py-3">
            <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2">
              <button
                type="button"
                onClick={() => setShowFilterPanel(prev => !prev)}
                className={`flex min-h-11 items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-bold shadow-sm transition-colors active:scale-[0.98] ${showFilterPanel ? 'border-blue-200 bg-blue-50 text-blue-700' : 'border-slate-200 bg-white text-slate-700 hover:bg-slate-50'}`}
                title="数据过滤设置"
              >
          ...
          {showFilterPanel && (
            <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-lg">
              <div className="mb-3 flex items-center justify-between border-b border-slate-100 pb-3">
                <div>
                  <h4 className="text-sm font-bold text-slate-800">数据过滤 / DATA FILTERS</h4>
                ...
                <button
                  ...
                  className="flex h-10 w-10 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-600 active:scale-95"
                  aria-label="关闭数据过滤面板"
                >
                  <X className="h-5 w-5" />
                </button>
                ...
                {([
                  ...
                  <label key={key} className="flex min-h-16 cursor-pointer items-start gap-3 rounded-xl border border-slate-200 bg-slate-50/40 p-3 text-xs text-slate-600 hover:bg-white active:scale-[0.99]">
                    ...
                    <span className="flex-1">
                      ...
                      <span className="mt-0.5 block text-[11px] leading-relaxed text-slate-400">{hint}</span>
                    </span>
                  </label>
                ))}
              </div>
